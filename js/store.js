$(function () {
  ...
  $.getJSON("./data/store.json", function (data) {
    ...
    function findCity(city, country) {
      ...
      let listHTML = "",
        ...
        filterdCountry = countries.filter(
          (item) => item.countryName == country
        ),
        filteredStore = filterdCountry[0].store;

      $.each(filteredStore, (i, item) => {
        listHTML += `<li class="mix" data-lat="${item.lat}" data-lng="${item.lng}" data-img="${item.image}">
    ...
    makeMap(
      ...
      127.0394159,
      "강남본점",
      "서울특별시 강남구 도산대로 403 LG전자 베스트샵 강남본점",
      "02-3448-5191",
      "https://www.lge.co.kr/kr/upload/admin/storePhoto/%EA%B0%95%EB%82%A8%EB%B3%B8%EC%A0%90%20%EC%99%B8%EA%B4%80-crop[20221214_095145].jpg"
    );
  ...
  function makeMap(lat, lng, mark_name, mark_address, mark_phone, mark_img) {
    ...
    var imageSrc =
      "https://res.cloudinary.com/dmtdo3wur/image/upload/v1691479488/j8lwiacazkjqmhdnbo7z.png";
